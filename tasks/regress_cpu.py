...
# command line argument: python3 regress_cpu.py sysinfo-train.csv sysinfo-valid.csv
...
def get_data(filename):
    ...
    sysinfo = sysinfo.dropna(subset=['next_temp'])
...
def get_trained_coefficients(X_train, y_train):
    ...
    Reg_Coefficients_=model.coef_
    coefficients=Reg_Coefficients_
...
def smooth_test(coef, sysinfo, outfile,title):
...
# def smooth_test_2(coef, sysinfo, outfile,title):
#     X_valid, y_valid = sysinfo[X_columns], sysinfo[y_column]
#     transition_stddev = 0.5
#     observation_stddev = 1.0
#     dims = X_valid.shape[-1]
...
#     kalman_smoothed, _ = kf.smooth(X_valid)
#     residuals = y_valid - kalman_smoothed[:, 0]
#     plt.figure()
#     plt.hist(residuals, bins=100)
#     plt.savefig(title)
#     plt.close()
...
def main(training_file, validation_file):
    ...
    # print('model valid score: ',model.score(X_valid,y_valid),'\n')
    output_regression(coefficients)
    smooth_test(coefficients, train, 'train.png','training data')
    ...
    smooth_test(coefficients, valid, 'valid.png','valid data')
    # smooth_test_2(coefficients, valid, 'residuals_valid.png','valid residuals')
    # smooth_test_2(coefficients, train, 'residuals_training.png','training residuals')
